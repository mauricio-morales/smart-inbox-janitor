...
public class ProviderBridgeService : IProviderBridgeService
{
    ...
    public async Task<Result<ProviderStatus>> GetEmailProviderStatusAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<ProviderStatus>(new ProviderError($"Status check failed: {ex.Message}"));
        }
    }
    ...
    private async Task<Result<string>> TestGmailConnectivityAsync()
    {
        try
        {
            ...
            return Result.Success(userEmail ?? "Connected");
        }
        catch (Exception ex)
        {
            return Result.Failure<string>(new ProviderError($"Gmail connectivity test failed: {ex.Message}"));
        }
    }
    ...
    private async Task<Result<bool>> TestOpenAIConnectivityAsync(string apiKey)
    {
        try
        {
            ...
            {
                return Result.Failure<bool>(new ProviderError("Invalid API key format"));
            }
        ...
        {
            return Result.Failure<bool>(new ProviderError($"OpenAI connectivity test failed: {ex.Message}"));
        }
    }
    ...
    private async Task<Result<Dictionary<string, object>>> TestStorageConnectivityAsync()
    {
        ...
        {
            return Result.Failure<Dictionary<string, object>>(new ProviderError($"Storage connectivity test failed: {ex.Message}"));
        }
    }
