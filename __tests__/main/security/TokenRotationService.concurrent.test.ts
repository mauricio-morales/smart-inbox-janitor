...
describe('TokenRotationService - Concurrent Scenarios', () => {
  ...
  beforeEach(async () => {
    ...
    mockSecureStorage = new MockSecureStorageManager({} as any) as jest.Mocked<SecureStorageManager>;
    ...
    mockGmailOAuth.refreshTokens.mockResolvedValue(createSuccessResult({
      ...mockGmailTokens,
  ...
  describe('Concurrent Rotation Prevention', () => {
    ...
    it('should handle concurrent rotation checks properly with lock', async () => {
      ...
      const checkMethod = (tokenRotationService as any).checkAndRotateTokens.bind(tokenRotationService);
      
      // Start multiple rotation checks concurrently
      const promises = Array(5).fill(0).map(() => checkMethod());
  ...
  describe('Timeout Protection', () => {
    it('should timeout long-running rotation operations', async () => {
      ...
      mockGmailOAuth.refreshTokens.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => 
          resolve(createSuccessResult(mockGmailTokens)), 35000) // 35 seconds, longer than 30s timeout
        )
      );
  ...
  describe('Atomic Status Updates', () => {
    it('should update rotation status atomically', async () => {
      ...
      // Access private method for testing
      const atomicUpdate = (tokenRotationService as any).atomicUpdateRotationStatus.bind(tokenRotationService);
